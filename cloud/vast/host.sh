#!/usr/bin/env bash
# Manage the single Vast.ai validation instance.  Mutating commands are explicit.
set -euo pipefail
...
Commands:
  search [--dry-run]
  create --offer-id ID --disk-gb GB [--image IMAGE] [--dry-run]
...
case "$command_name" in
    search)
        ...
        run vastai search offers \
            'gpu_name in [RTX_3090,RTX_A5000,RTX_A6000,A100] num_gpus=1 gpu_ram>=24 cpu_cores>=8 cpu_ram>=32 disk_space>=500 reliability>=0.99 direct_port_count>=1 verified=true rentable=true' \
            --type on-demand -o 'dph_total'
        ;;
    create)
        ...
        require_vastai
        run vastai create instance "$offer_id" --image "$image" --disk "$disk_gb" --ssh --direct
        ;;
    status)
        ...
        require_vastai
        run vastai show instance "$instance_id"
        ;;
    ssh-url)
        ...
        require_vastai
        run vastai ssh-url "$instance_id"
        ;;
    stop)
