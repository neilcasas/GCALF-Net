...
prostate volume. The Swin Transformer branch captures long-range dependencies and contextual relationships by applying self-attention mechanisms across the volumetric space. In the PDHD-Net, the outputs of the two branches are fused using the Window-based self-attention fusion (WAF) module, where features are concatenated and processed through window-based self-attention. This permits feature integration, while cross-attention offers a possible directed interaction between branches. Wang et al. (2025) report per-grade FROC detection sensitivities, not confusion-matrix recall for GGG classification; those figures do not establish a GGG2/GGG3 classification weakness or show that WAF caused one. This study therefore treats cross-attention as a design hypothesis to evaluate, rather than a demonstrated correction to a reported PDHD-Net failure.
...
Wang et al. (2025) report FROC detection sensitivity stratified by ground-truth grade, including 96.7% for GGG 5 at one false positive per patient; these are lesion-detection sensitivities, not grade-classification recall. Their separate ROC result is an AUC of 0.824 for binary discrimination of clinically significant cancer (GGG ≥2) from GGG 1. The paper therefore does not establish its multi-class GGG2/GGG3 classification performance. Differentiating GGG 2 (Gleason 3+4) from GGG 3 (Gleason 4+3) remains a clinically important objective of the present study, but it is not presented here as a weakness demonstrated by those PDHD-Net detection results.
...
The second modification addresses the feature fusion mechanism in the ConvSwin3D encoder. In Wang et al. (2025), CNN and Swin Transformer features are concatenated and processed using window-based self-attention. Cross-attention can instead provide directed interaction in which one branch queries the other. The paper's per-grade FROC values measure detection sensitivity at a false-alarm operating point and do not show lower GGG2/GGG3 classification recall; its reported AUC concerns the binary GGG ≥2 versus GGG1 endpoint. The present study evaluates cross-attention as a proposed mechanism for its lesion-level GGG2–5 task, without attributing an unreported classification failure to PDHD-Net.
...
Based on Figure 2.4, Wang et al. (2025) introduced the Prostate Dual-branch Hybrid Domain prediction Network (PDHD-Net) to achieve fine-grained stratification of the ISUP Gleason Grade Group (GGG 1-5). The PDHD-Net leverages a parallel dual-branch encoder that integrates a multi-stage 3D CNN with a Swin-Transformer, enhanced by an innovative frequency-band-shunting feature-enhancement strategy. This strategy directs low-frequency MRI components to the Swin-Transformer to capture global macro-structures, while high-frequency components are routed to the CNN to accentuate local lesion contours. Its per-grade FROC sensitivities, including the reported 96.7% for GGG5, describe lesion detection at selected false-alarm rates; they are not grade-classification recall. The paper's separate ROC result reports AUC 0.824 for binary GGG ≥2 versus GGG1 classification, so its results do not establish multi-class GGG2–5 performance or a GGG2/GGG3 classification weakness.

36 

Figure 5 reports per-grade FROC detection sensitivities at 0.5 false alarms per patient: GGG1 85.6%, GGG2 85.7%, GGG3 87.3%, GGG4 83.3%, and GGG5 96.7%. These values measure detection, not classification recall.
...
Wang et al. (2025), in the original PDHD-Net, fused CNN and Swin Transformer features using window-based self-attention applied after concatenation. Although this allows information mixing within a single sequence, it treats both feature streams symmetrically and does not explicitly model the directed, asymmetric query–key–value exchange that cross-attention can provide between branches. The reported per-grade values are FROC detection sensitivities and do not establish lower GGG2/GGG3 classification recall or show that this fusion mechanism is inadequate for that boundary. The present study tests cross-attention as a design hypothesis for lesion-level grading.
...
For the separate PDHD-Net comparison, results are described as a **harmonized FROC-style comparison**, not an exact reproduction. The comparison reports generic detection sensitivity stratified by supervised ground-truth grade and grade-specific candidate sensitivity, where the candidate score is detection confidence multiplied by the corresponding GGG probability. Both use the repository's shared score-ordered IoU matcher at 0.5 IoU and are reported at 0.5 and 1 false positives per patient. PDHD-Net's per-grade FROC values are detection sensitivities, not grade-classification recall; its reported AUC of 0.824 is for binary GGG ≥2 versus GGG1 classification.
