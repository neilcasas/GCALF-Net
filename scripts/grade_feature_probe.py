...
from gcalf_eval.grade_pilot import bootstrap_metrics, make_split_manifest, patient_id
...
def validate_export(features, grades, supervised_mask, case_ids):
    ...
    return features, grades.astype(np.int64, copy=False), supervised_mask, case_ids.astype(str, copy=False)
...
def main():
    ...
    with np.load(args.feature_file, allow_pickle=False) as archive:
        features, grades, supervised, case_ids = validate_export(
    ...
    fit_features = features[rows][fit_mask]
    fit_grades = grades[rows][fit_mask]
    eval_features = features[rows][evaluation_mask]
    ...
    _, _, point_estimate, ci, valid_resamples = _fit_and_bootstrap(
        fit_features, fit_grades, eval_features, eval_grades, eval_patients,
        seed=SEED, samples=BOOTSTRAP_SAMPLES)
    lower_bound = ci[0]
    record = {
        "schema_version": 1,
        "protocol": "ADR 0005 Day-0 patient-disjoint multinomial logistic regression",
        ...
        "bootstrap_seed": SEED,
        "interpretation": "supports proceeding" if lower_bound > 0.5 else "null expectation",
        "locked_build_unchanged": True,
    }
    (args.output_dir / "probe_result.json").write_text(json.dumps(record, indent=2) + "\n")
    print(json.dumps({key: record[key] for key in (
        "point_estimate", "patient_bootstrap_95_ci", "bootstrap_valid_resamples", "interpretation")}, indent=2))
